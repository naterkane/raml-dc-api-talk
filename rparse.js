...
raml.composeFile('api-doc/api.raml').then(function(rootNode) {
  ...
  _.forOwn(doclines,function(line) {
    ...
    if (line.match(/\!include\s(examples|schemas)\/\w+\-*\w+(\.json)/)) {
      ...
      include = fs.readFileSync('api-doc/' + line.match(/(examples|schemas)\/\w+\-*\w+(\.json)/)[0], 'utf8');
      /**
  ...
  //if (fs.file)
  fs.writeFile('api-doc/api-flat.raml',newdoc.join('\n'), 'utf8',function(err) {
        if (err) {
          console.warn(err);
          //exit;
        }
